...
interface ViewerComponent : KoinComponent {
...
class DefaultViewerComponent(
    ...
    override val state: MutableValue<ViewerComponent.State> = MutableValue(
...
//    }
    ...
    override fun readChapter(index: Int) {
        ...
        root.navigateToReader(
            state.value.sourceId, state.value.preview.id, index,when(state.value.data is StoredManga){
                true -> listOf()
                else -> state.value.chapters
            }
        )
    }
